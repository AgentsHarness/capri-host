...
| `PROXY` | — | 出网代理，可只写 `host:port`。同时导出给 grok agent（模型 API 请求走它） |
| `NO_PROXY` | — | 不走代理的地址列表 |
...
也能让 host 中继和它拉起的 grok agent 走代理——agent 请求模型 API 走的就是这组变量。留空则不设置，
沿用父进程原有的代理环境变量（命令行 / launchd 已经设过 `HTTPS_PROXY` 的，不会被覆盖）。
